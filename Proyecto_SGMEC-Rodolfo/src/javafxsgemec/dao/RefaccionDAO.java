...
public class RefaccionDAO {    
    public static ArrayList<Refaccion> consultRefaccionesProovedor(int idProveedor){
        ...
        if(conexionBD != null){
            try {
                String consulta = "SELECT Refaccion.idRefaccion, Refaccion.nombreRefaccion, "
                        + "TipoRefaccion.nombreTipoRefaccion, RefaccionProveedor.pzasDisponiblesCompra, "
                        + "RefaccionProveedor.precioCompra, Proveedor.idProveedor FROM Refaccion "
                        + "INNER JOIN TipoRefaccion on TipoRefaccion.idTipoRefaccion = "
                        + "Refaccion.idTipoRefaccion INNER JOIN RefaccionProveedor on RefaccionProveedor.idRefaccion "
                        + "= Refaccion.idRefaccion INNER JOIN Proveedor on Proveedor.idProveedor = "
                        + "RefaccionProveedor.idProveedor WHERE Proveedor.idProveedor = ? AND "
                        + "RefaccionProveedor.pzasDisponiblesCompra > 0;";
    ...
    public static ResultOperation modificarPzasDisponiblesCompraRefaccion(Refaccion refaccionModificada, int idProveedor) throws SQLException{
        ResultOperation respuesta = new ResultOperation();
        respuesta.setError(true);
        respuesta.setNumberRowsAffected(-1);
        ...
        if(conexionBD != null){
            try {
                ...
                if(numeroFilas > 0){
                    ...
                    respuesta.setMessage("Informacion de refaccion modificada correctamente");
                    respuesta.setNumberRowsAffected(numeroFilas);
                }else{
                    respuesta.setMessage("No se pudo modificar la informacion de refaccion.");
                }
